...
object MovieList {
    ...
    private fun buildMovieInfo(
        ...
        movie.exo = if (SDK_VER < OLD_SDK_VERSION)
            func.contains("^nowtv|^fantv$|^rthk$|^fox$|^fb$|^wowgua|^in$|^tw$|^exoCustom|^ggiptv_ty_6$".toRegex())
        else
            !func.contains("^custom".toRegex())
        movie.fixRatio = func.contains("^nowtv630$|^in$|^mocable$|^gdtv_fix$".toRegex()) || videoUrl.contains("".toRegex()) || title in "遼寧體育"
